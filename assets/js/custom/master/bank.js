$(document).ready(function() {
	...
	$("#tblBank").DataTable({
		...
		//load data
		ajax: {
			url: base_url+'master/bank/ajax_list',
			type: 'POST',
		},
...
function reloadTable() {
	$("#tblBank").DataTable().ajax.reload(null,false);
}
...
$(document).on('click', '#btnTambah', function (e) {
	...
	$("#formData")[0].reset();
	$(".modal-title").text("Tambah Master Bank");
...
function btnEdit(id) {
	idData = id;
	$("#modalForm").modal("show");
	$(".modal-title").text("Update Master Bank");
	save_method = "update";

	$.post(base_url+"master/bank/getId/"+idData,function(json) {
		if (json.status == true) {
			$('#kode').val(json.data.kode);
		    $("#bank").val(json.data.bank);
...
$("#modalButtonSave").click(function() {
	var url;
	if (save_method == "add") {
		url = base_url+'master/bank/add';
	} else {
		url = base_url+'master/bank/update/'+idData;
	}
	...
	$.ajax({
		...
		success: function(json) {
			if (json.status == true) {
				...
				$("#errorbank").html(json.error.bank);
	...
	$.post(base_url+"master/bank/getId/"+idData,function(json) {
		if (json.status == true) {
			...
			var pesan = "<hr>";
				...
				pesan += "<li class='pull-left'><small>bank : <i>"+json.data.bank+"</i></small></li><br>";
		    ...
		    swal({   
		    	...
		    	if (result.value) {
		    		$.post(base_url+"master/bank/delete/"+idData,function(json) {
						if (json.status == true) {
							swal({    
						            title: json.message,
						            type: "success",
