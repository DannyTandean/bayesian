$(document).ready(function() {
	...
	$("#tblDepartemen").DataTable({
		...
		//load data
		ajax: {
			url: base_url+'master/departemen/ajax_list',
			type: 'POST',
		},
...
$(document).on('click', '#btnTambah', function (e) {
	...
	$("#formData")[0].reset();
	$(".modal-title").text("Tambah Master Departemen");
	save_method = "add";
});
...
function btnEdit(id) {
	idData = id;
	$("#modalForm").modal("show");
	$(".modal-title").text("Update Master Departemen");
	save_method = "update";

	$.post(base_url+"master/departemen/getId/"+idData,function(json) {
		if (json.status == true) {
...
$("#modalButtonSave").click(function() {
	var url;
	if (save_method == "add") {
		url = base_url+'master/departemen/add';
	} else {
		url = base_url+'master/departemen/update/'+idData;
	}
	...
	$.post(base_url+"master/departemen/getId/"+idData,function(json) {
		if (json.status == true) {
		    ...
		    swal({   
		    	...
		    	if (result.value) {
		    		$.post(base_url+"master/departemen/delete/"+idData,function(json) {
						if (json.status == true) {
							swal({    
						            title: json.message,
						            type: "success",
