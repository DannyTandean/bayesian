$(document).ready(function() {
	...
	$("#tblRewardApproval").DataTable({
		...
		//load data
		ajax: {
			url: base_url+'/approvalowner/reward/ajax_list',
			type: 'POST',
		},
...
function reloadTable() {
	$("#tblRewardApproval").DataTable().ajax.reload(null,false);
}
...
function btnApprove(id) {
	idData = id;
	$.post(base_url+"approvalowner/reward/getId/"+idData,function(json) {
		$("#status").val("Diterima");
		if (json.status == true) {
			if(json.data.status == "Proses") {
				var pesan = "<hr>";
					...
					pesan += "<li class='pull-left'><small>Nilai : <i>"+json.data.nilai+"</i></small></li><br>";
			    ...
			    swal({
					...
					if (result.value) {
						$.ajax({
							url: base_url+"approvalowner/reward/update/"+idData,
							type:'POST',
