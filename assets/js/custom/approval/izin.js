$(document).ready(function() {
	...
	$("#tblIzinApproval").DataTable({
		...
		//load data
		ajax: {
			url: base_url+'approval/izin/ajax_list',
			type: 'POST',
		},
...
function reloadTable() {
	$("#tblIzinApproval").DataTable().ajax.reload(null,false);
}
...
function btnApprove(id) {
	idData = id;
	$.post(base_url+"approval/izin/getId/"+idData,function(json) {
		$("#status").val("Diterima");
		if (json.status == true) {
			...
			if(json.data.status == "Proses") {
			    ...
			    swal({
					...
					if (result.value) {
						$.ajax({
								url: base_url+"approval/izin/update/"+idData,
								type:'POST',
