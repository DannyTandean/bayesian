...
class Jadwalkerja_model extends CI_Model {
	...
	{
		parent::__construct();
		$this->_table = "jadwal_token_karyawan";
		$this->_primary_key = "id";
	}

	public function setQueryDataTable($select,$where,$search,$groupBy=true,$otoritas=true)
	{
		...
		if ($groupBy) {
			$this->db->group_by(array('jadwal_token_karyawan.tanggal','group_id','nama_shift'));
		}
		$this->db->where($where);
		if ($otoritas) {
			$this->db->where_not_in('otoritas', array(2,4));
		}
	...
	public function findDataTable($select,$where,$columnsOrderBy,$search,$groupBy=true,$otoritas=true)
	{
		$input = $this->input;
		$orderBy = false;
		...
		// query data table
		self::setQueryDataTable($select,$where,$search,$groupBy,$otoritas);
	...
	public function findDataTableOutput($data=null,$select,$where,$search,$groupBy=true,$otoritas=true)
	{
		$input = $this->input;
	...
	{
		$this->db->select("jadwal_token_karyawan.karyawan_id, jadwal_token_karyawan.tanggal, jadwal_token_karyawan.group_id, jadwal_token_karyawan.shift_id, jadwal_token_karyawan.nama_shift, master_karyawan.foto, master_karyawan.nama, master_karyawan.idfp, master_karyawan.tgl_lahir, master_karyawan.kelamin, master_jabatan.jabatan, master_grup.grup, ganti_karyawan_id");
		$this->db->where('jadwal_token_karyawan.id',$id);
		$this->db->from($this->_table);
		$this->db->join('master_karyawan', 'master_karyawan.id = jadwal_token_karyawan.karyawan_id', 'left');
		$this->db->join('master_jabatan', 'master_jabatan.id_jabatan = master_karyawan.id_jabatan', 'left');
		$this->db->join('master_grup', 'master_grup.id_grup = master_karyawan.id_grup', 'left');
	...
	public function getCountKaryawan($where=false) // group dan tanggal
	{
		if ($where) {
			$this->db->where($where);
		}
